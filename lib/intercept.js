...
function activate() {
  ...
  common.overrideRequests(function (proto, overriddenRequest, args) {
    ...
    if (isOn() && interceptors) {
      ...
      const playbackReq = new http.ClientRequest(options, callback);

      // Save the second layer options so we can use it if request fails on matching the body
      playbackReq._secondLayerOptions = {
        secondLayer,
        proto,
        options,
        callback,
